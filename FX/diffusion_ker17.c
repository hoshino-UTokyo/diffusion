...
void allocate_ker17(REAL **buff_ret, const int nx, const int ny, const int nz) {

  posix_memalign((void**)buff_ret, 64, sizeof(REAL)*nx*ny*nz);

}

void init_ker17(REAL *buff1, const int nx, const int ny, const int nz,
...
void diffusion_ker17(REAL *restrict f1, REAL *restrict f2, int nx, int ny, int nz,
...
/*     } */
	...
	for (y = ystr; y < yend; y++) {
	  ...
	  s = (y == ny-1) ? 0 :   nx;
	  c =  y * nx + z * nx * ny;
	  svfloat64_t fc_vec0,fc_vec1,fc_vec2;
	  ...
	  fc_vec2  = svld1(pg,(float64_t*)&f1_t[c+16]);
