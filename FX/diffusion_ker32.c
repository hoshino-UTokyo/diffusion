...
void allocate_ker32(REAL **buff_ret, const int nx, const int ny, const int nz) {

  posix_memalign((void**)buff_ret, 64, sizeof(REAL)*nx*ny*nz);

}

void init_ker32(REAL *buff1, const int nx, const int ny, const int nz,
...
void diffusion_ker32(REAL *restrict f1, REAL *restrict f2, int nx, int ny, int nz,
...
/*     } */
	...
	for (y = ystr; y < yend; y++) {
	  ...
	  fc_vec0  = svld1(pg,(float64_t*)&f1_t[c+s   ]);fc_vec1  = svld1(pg,(float64_t*)&f1_t[c+s+ 8]);
	  fc_vec2  = svld1(pg,(float64_t*)&f1_t[c+s+16]);fc_vec3  = svld1(pg,(float64_t*)&f1_t[c+s+24]);
	  tmp0     = svmad_x(pg,cs_vec,fc_vec0,tmp0);    tmp1     = svmad_x(pg,cs_vec,fc_vec1,tmp1);
	  tmp2     = svmad_x(pg,cs_vec,fc_vec2,tmp2);    tmp3     = svmad_x(pg,cs_vec,fc_vec3,tmp3);
	  fc_vec0  = svld1(pg,(float64_t*)&f1_t[c+n   ]);fc_vec1  = svld1(pg,(float64_t*)&f1_t[c+n+ 8]);
	  fc_vec2  = svld1(pg,(float64_t*)&f1_t[c+n+16]);fc_vec3  = svld1(pg,(float64_t*)&f1_t[c+n+24]);
	  tmp0     = svmad_x(pg,cn_vec,fc_vec0,tmp0);    tmp1     = svmad_x(pg,cn_vec,fc_vec1,tmp1);
	  tmp2     = svmad_x(pg,cn_vec,fc_vec2,tmp2);    tmp3     = svmad_x(pg,cn_vec,fc_vec3,tmp3);
	  fc_vec0  = svld1(pg,(float64_t*)&f1_t[c+b   ]);fc_vec1  = svld1(pg,(float64_t*)&f1_t[c+b+ 8]);
	  fc_vec2  = svld1(pg,(float64_t*)&f1_t[c+b+16]);fc_vec3  = svld1(pg,(float64_t*)&f1_t[c+b+24]);
	  tmp0     = svmad_x(pg,cb_vec,fc_vec0,tmp0);    tmp1     = svmad_x(pg,cb_vec,fc_vec1,tmp1);
	  tmp2     = svmad_x(pg,cb_vec,fc_vec2,tmp2);    tmp3     = svmad_x(pg,cb_vec,fc_vec3,tmp3);
	  fc_vec0  = svld1(pg,(float64_t*)&f1_t[c+t   ]);fc_vec1  = svld1(pg,(float64_t*)&f1_t[c+t+ 8]);
	  fc_vec2  = svld1(pg,(float64_t*)&f1_t[c+t+16]);fc_vec3  = svld1(pg,(float64_t*)&f1_t[c+t+24]);
	  tmp0     = svmad_x(pg,ct_vec,fc_vec0,tmp0);    tmp1     = svmad_x(pg,ct_vec,fc_vec1,tmp1);
