...
#define ZB (3+HALO-1)
#define STEP 4
#define TB (HALO*(STEP-1))
#define SIMDLENGTH 8

void allocate_ker29(REAL **buff_ret, const int nx, const int ny, const int nz) {

  posix_memalign((void**)buff_ret, 64, sizeof(REAL)*nx*ny*nz);

}

void init_ker29(REAL *buff1, const int nx, const int ny, const int nz,
...
void diffusion_ker29(REAL *restrict f1, REAL *restrict f2, int nx, int ny, int nz,
...
#pragma omp parallel
  {
   ...
   //    int hst = TB;
    ...
    REAL *temporal;
    temporal = (REAL*)malloc(sizeof(double)*(STEP-1)*(2*HALO+1)*(YBF+2*TB)*(XBF+2*TB));
    int tbx = (XBF+2*TB);
    int tby = (YBF+2*TB);
    int tbz = (2*HALO+1);
...
#pragma statement cache_sector_size 0 4 12 4
