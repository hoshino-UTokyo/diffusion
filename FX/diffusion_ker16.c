...
void allocate_ker16(REAL **buff_ret, const int nx, const int ny, const int nz) {

  posix_memalign((void**)buff_ret, 64, sizeof(REAL)*nx*ny*nz);

}

void init_ker16(REAL *buff1, const int nx, const int ny, const int nz,
...
void diffusion_ker16(REAL *restrict f1, REAL *restrict f2, int nx, int ny, int nz,
...
/*     } */
	...
	for (y = ystr; y < yend; y++) {
	  ...
	  s = (y == ny-1) ? 0 :   nx;
	  c =  y * nx + z * nx * ny;
	  svfloat64_t fc_vec0,fc_vec1,fc_vecm,fc_vecp;
	  ...
	  float64_t fc_vectors[32];
	  fc_vec0  = svld1(pg,(float64_t*)&f1_t[c]);
	  fc_vecm  = fc_vec0;
	  fc_vec1  = svld1(pg,(float64_t*)&f1_t[c+8]);
	  fc_vecp  = svld1(pg,(float64_t*)&f1_t[c+16]);
	  svst1(pg,(float64_t*)&fc_vectors[ 0],fc_vecm);
	  svst1(pg,(float64_t*)&fc_vectors[ 1],fc_vecm);
	  svst1(pg,(float64_t*)&fc_vectors[ 8],fc_vec0);
	  svst1(pg,(float64_t*)&fc_vectors[16],fc_vec1);
	  svst1(pg,(float64_t*)&fc_vectors[24],fc_vecp);
	  fce_vec0 = svld1(pg,(float64_t*)&fc_vectors[ 9]);
	  fce_vec1 = svld1(pg,(float64_t*)&fc_vectors[17]);
	  fcw_vec0 = svld1(pg,(float64_t*)&fc_vectors[ 0]);
	  fcw_vec1 = svld1(pg,(float64_t*)&fc_vectors[15]);
	  ...
	  tmp6     = svmul_x(pg,cc_vec,fc_vec0);  tmp7     = svmul_x(pg,cc_vec,fc_vec1);
	  ...
	  tmp0 = svadd_x(pg,tmp6    ,tmp0    ); tmp1 = svadd_x(pg,tmp7    ,tmp1    );
	  ...
	  for (x = 16; x < nx-16; x+=16) {
	    ...
	    fce_vec0 = svld1(pg,(float64_t*)&fc_vectors[ 9]);
	    fce_vec1 = svld1(pg,(float64_t*)&fc_vectors[17]);
	    fcw_vec0 = svld1(pg,(float64_t*)&fc_vectors[ 7]);
	    fcw_vec1 = svld1(pg,(float64_t*)&fc_vectors[15]);
	  ...
	  svst1(pg,(float64_t*)&fc_vectors[ 8],fc_vec0);
	  svst1(pg,(float64_t*)&fc_vectors[24],fc_vecp);
	  svst1(pg,(float64_t*)&fc_vectors[23],fc_vecp);
	  svst1(pg,(float64_t*)&fc_vectors[16],fc_vec1);
	  fce_vec0 = svld1(pg,(float64_t*)&fc_vectors[ 9]);
	  fce_vec1 = svld1(pg,(float64_t*)&fc_vectors[24]);
	  fcw_vec0 = svld1(pg,(float64_t*)&fc_vectors[ 7]);
	  fcw_vec1 = svld1(pg,(float64_t*)&fc_vectors[15]);
